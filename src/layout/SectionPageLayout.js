...
const SectionPageLayout = ({ title, isDark, bgLightDark, icon, children, label, idLabel,  bgColor, align, sectionMaxWidth, bigTitle}) => {
    const checkIcon = (icn) => {
        if ( icn === "Check Mark" ) {
    ...
    return (
        <section id={idLabel} className={label} style={{ backgroundColor: bgColor }}>
            <div className={`section-title ${isDark ? 'darkTitle' : '' } sectionMaxWidth ${sectionMaxWidth ? 'sectionMaxWidth' : ''}`}>
                <motion.h2 initial="hidden" animate="show" exit="exit" variants={titleTextFadeIn} style={{ textAlign :  align ? 'left' : 'center', fontSize : bigTitle ? 'clamp(5rem, 15vw, 20rem)' : ''}}>
